...
├── movies.json                 # Movie config: list of {slug, enabled, embargo_lift_date?, theatrical_release_date?} objects
...
├── tests/
│   └── test_rotten_tomatoes.py # 104 tests (all pure logic, no network/browser)
├── .github/
│   └── workflows/
...
│   ├── plans/                  # Feature/migration plan docs
│   │   ├── backfill_anti_blocking.md
│   │   ├── backfill_date_parsing.md
│   │   └── ...                 # (numbered infra plans, decisions, etc.)
...
│       └── silent_zero.md      # Silent-zero detection (0-cards gating by config dates)
...
- **Silent-zero severity gate**: A scrape returning 0 reviews can be legitimate (pre-embargo) or a silent failure (selector change, bot wall). Two optional `movies.json` fields disambiguate: past `embargo_lift_date` → WARNING, past `theatrical_release_date` → ERROR (fires the Cloud Run alert). Before either date, or if neither is set, → INFO only. `theatrical_release_date` is soft-required for enabled entries: missing it logs ERROR at config load but the entry still scrapes (one bad entry doesn't block others). Dates are midnight Eastern Time.
...
- **`scrape(movie_entry)`** -- Orchestrates scraping for a single movie (takes dict with slug + optional date fields). Two-pass (top-critics, all-critics). "Connect late" pattern: scrapes into memory, connects to Neon only for batch insert. Spike guard rolls back if insert count exceeds threshold (possible selector breakage). If scraping returns 0 reviews total, calls `_log_no_reviews()` to log INFO/WARNING/ERROR based on config dates, then returns before the DB connect.
...
- **`_build_driver(js_heap_mb=256)`** -- Headless Chromium with `--no-sandbox`, `--disable-dev-shm-usage`, `--js-flags=--max-old-space-size={js_heap_mb}`, `--disable-blink-features=AutomationControlled`, realistic User-Agent (`Chrome/134.0.0.0`). Page load timeout 30s, script timeout 15s. Backfill overrides to `js_heap_mb=1024` for large DOMs.
...
- **Multi-movie config** -- `movies.json` with `[{slug, enabled, embargo_lift_date?, theatrical_release_date?}]` entries. `load_movie_config()` returns list of dicts for enabled entries, parsing date fields as midnight Eastern Time. Soft-enforces `theatrical_release_date` for enabled entries: missing it logs ERROR at config load (entry still returned). Invalid date format logs WARNING and degrades to None. Also WARNs if `embargo_lift_date > theatrical_release_date` (likely typo). CLI `--movie <slug>` overrides config (constructs entry with None dates, so silent-zero gate stays at INFO).
- **Silent-zero gate** -- `_log_no_reviews(slug, embargo, release, now)` called from `scrape()` when both critic filters return 0 reviews. Past `theatrical_release_date` → ERROR. Past `embargo_lift_date` (not yet past release) → WARNING. Otherwise INFO. Runs before DB connect so silent-zero movies don't trigger a Neon cold start. Error playbook: K-1 through K-9 in `planning/errors/silent_zero.md`.
...
# Backfill all movies from CSV (with per-movie time cutoffs from time_end column)
